...
| 42 | Official-source refresh + m=9 monitoring (2026-09-06; `src/pcso_monitoring_run.py`, 128 draws `official_verified`, 834/834 prior rows re-verified on pcso.gov.ph) | per-game MC χ², moon-alt/illum perm. r, #45 binomial on n=186 confirmation draws | uniform / r=0 / 6/55 | χ² p 0.046–0.50; r −0.015/+0.005; #45 2 of 37 | min p 0.046 > 0.0056 — **0 flags** (G0 repeated-look monitoring; RESULTS_PCSO_REFRESH_2026-09-06.md §2) |
| 43 | Conscious selection / jackpot sharing (kb card 28, decision layer; `src/csi_popularity.py`, 984 official draws, within-game stratified; r2 after codex review) | T1 = Δ standardized CSI (winner vs no-winner draws); T2 = rank corr(CSI, winning bets); Poisson pseudo-MLE slope (HC1) | 0 / 0 (null trial 0.001±0.112 / 0.000±0.031) | +0.736 sd / +0.162; count ratio per SD 1.68 [1.44, 1.95] | **Holm p 1e-04 / 1e-04** — popularity predicts official jackpot sharing; a claim about PLAYERS, not the draw; EXPLORATORY G0 |
| 44 | Draw-paired strategy backtest (`src/pcso_strategy_backtest.py`, 834 draws × 2 tickets, conditional MC null with tickets fixed, Holm m=7; r2) | Δ matches/ticket vs uniform on the same draws | 0 | v1 +0.0040 (p .0078, Holm .055), v2 +0.0081 (.013, .067), cold −0.041 (.011, .064), hot/overdue/repeat/Markov n.s. | **no rule differs from uniform after Holm**; the first-pass "one flag, charged once" reading withdrawn; G0 |
| 45 | Next-draw posterior predictive (`src/pcso_next_draw_posterior.py`, product-weight 6-without-replacement model, Dirichlet(100) fixed prior, 962 draws) | R = C(P,6)·E[f_w(S*) \| D] for the maximum-predictive set; BF₀₁ uniform:tilted; order-1 overlap exact test | R=1, Δ=0 exactly under uniform | R 1.48–1.64, CrI lower ends 0.98–1.08; BF₀₁(100) 0.22–0.95, (10) 46–6976; R_mix 1.25–1.52; overlap p ≥ 0.041 | evidence for unequal weights weak/prior-sensitive (BF₀₁<1 at a=100 in all games, reversed at a=10); tested serial-dependence statistics do not reject at Bonferroni 0.01; codex gpt-6-astra reconstruction reaches the same conclusions with third-decimal differences; G0 |
| 41 (order kept: rows 42–44 above were appended 2026-09-06) | Registered rerun batch67_r2 (hash 2709deedb274f0f6, first G3 batch; haiku execute-only; src/core+src/domains) | b6_mmd m=1199 min_p=0.049; b6_spectra m=1199 min_p=0.036; b6_halves m=199 3 regimes (6/55 all 0.251/ex-susp 0.154/ver-only 0.177); b7_seasons m=399 6/6 p=0.0025; b7_cca 3-split sun-moon ρ₁ 0.403–0.567 all p=0.005; Kp all null; b7_gw G0 only | floor-compliant m, Šidák per family | all original verdicts resolution-stable | b6 joint NULL ×3; b7 seasons REJECT ×6 strengthened; pressure↔sun-moon REJECT ×3 splits strengthened; Kp NULL; GW G0 no claims; panel corrected to n=136 KS p=0.111, 8.8% ≤0.05 (double-count artifact KS p=0.027 resolved) — RESULTS_BATCH6_7_RERUN.md |
