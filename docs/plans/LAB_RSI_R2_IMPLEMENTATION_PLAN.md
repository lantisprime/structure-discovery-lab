...
| REQ-2 | Class scope is enforced in code: `lab_heal.CLASS_SCOPE` maps each registry class (`agent`, `instrument`, `theorem_card`, `adapter_manifest`, `suite`) to allowed path prefixes; `HEAL_NOTES.md` and the record dir are always allowed. A tree with any change outside the scope of the defect's class is `REJECTED` at a new stage `scope` **before** `./tools/check.sh` runs (no gate cost on an out-of-scope proposal). *(Review finding 1:)* the same stage rejects any modified or deleted **tracked** file under `results/` (`frozen_results`: a frozen result or a historical record is never rewritten; new files are allowed) and any touch of another dispatch record under `results/agent_runs/` (`audit_records`); the R3 gate independently rejects a PR whose diff modifies a tracked non-`.jsonl` file under `results/` (`results_modified`). A note containing `OWNER-RESERVED` is likewise recorded as `REJECTED` at stage `owner-reserved` without running the gate; the attempt still counts, so the R3 gate routes the occurrence to the owner at the attempt cap (faster routing of a deliberate stop is an R3 follow-up). | `test_heal_rejects_out_of_scope_changes_before_gate`, `test_heal_records_owner_reserved_stop_without_running_the_gate` | MUST |
...
| Live: `--stale` after the review (REQ-6/REQ-7) | edit `agents/lab-proposer.md` (review finding 1 wording) → `outcome_collect.py --sources agent_eval` → `agent_eval_dispatch.py --stale` → collect | collector: `STALE_EVAL … changed since eval record commit 905d3c7; no eval pass, no dispatch` for P-1 and P-2 (the healer would now refuse to run); `--stale`: "definition STALE_EVAL against its record -> dispatching" ×2 → P-1 **PASS** (`eval-p1-20260907T225619`), P-2 **PASS** (`eval-p2-20260907T225741`); the next collect read the new records. Finding folded in: an uncommitted fresh record has no record commit, so the collector compared it with the *previous* row's hash and still said stale — it now reads the definition hash the dispatcher stamped into `agent.txt` first |
...
Second-opinion review 2026-09-08: read-only Sonnet reviewer over
`master...feature/rsi-r2-proposer` (codex remains preflight-gated from
Claude sessions, as in R3). Verdict **APPROVE-WITH-CHANGES**; disposition:

| # | Finding | Severity | Disposition |
|---|---|---|---|
| 1 | `CLASS_SCOPE["instrument"]` admits all of `results/`, so a heal could rewrite a historical dispatch record or another instrument's frozen result, and the gate's mechanical `scope_reasons` only sees `.jsonl` line loss and test deletions | BLOCK | **ACCEPT**: `lab_heal.scope_violations` now rejects, before the gate, any modified/deleted *tracked* file under `results/` (`frozen_results`) and any touch of another record under `results/agent_runs/` (`audit_records`); `lab_gate.diff` records `results_modified` (`--diff-filter=MD` under `results/`, `.jsonl` excluded as ledger territory) and `scope_reasons` rejects on it; definition and brief say "results/: new files only". Tests: heal (rewriter, meddler, adder cases), real-git gate diff, FakeGH gate reject; the heal test fixture now plants its defect in the instrument, not the data file |
| 2 | `lab_loop.sh` claims a modified tracked record is "reported" but silently drops it | SHOULD | **ACCEPT**: `git diff --name-only -- results/agent_runs` non-empty → print paths, `exit 5` (as the ledger-deletion check) |
| 3 | `test_grading_never_mutates_records` covered only the 2026-06 floors | SHOULD | **ACCEPT**: iterates floors ∪ `record_dirs()` |
| 4 | `record_slug` would collide `D-1` with `D-1+D-2` | NIT | **ACCEPT-WITH-MOD**: naming kept (matches the 2026-06 dirs); a test asserts slugs are unique across `RECORDS`; docstring warns |
| 5 | P floors named a directory that never existed | NIT | **ACCEPT**: floors point at the first PASS records at the committed tier; a test asserts they exist |
| 6 | `Bash(python3:*)` in the headless commands can reach the network although `curl`/`ssh`/`gh` are disallowed; pre-existing in R4, now duplicated into a routine loop step | SHOULD | **DEFER** (tracked): an egress control belongs to the environment the loop runs in (the lab machine / launchd), not to an allow-list of binaries; carried to change set 8 with the scheduler install as an owner item |
| — | `parse_verdict`, staleness close, `record_dir`, `--stale` throttle, append-only in this diff, secret redaction | — | no findings |
