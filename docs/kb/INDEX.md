...
| 28 | [conscious-selection-popularity.md](conscious-selection-popularity.md) | decision | Stern–Cover/Cook–Clotfelter conscious selection: payout, not P(win), depends on combination popularity; declared CSI proxy; null trial + first run on 984 official draws (`src/csi_popularity.py`, family `payout-sharing`, m=2). **EXPLORATORY G0** (2026-09-06). |
| 29 | [arxiv-stochastic-prediction-survey.md](arxiv-stochastic-prediction-survey.md) | statistical | arXiv + classics survey for the picker question; verdict: no surveyed theorem raises P(pick 6/6); adopted A4 anytime-valid e-process monitor (`src/pcso_eprocess_monitor.py`, v2 mixture + e-detector). Literature survey, G-labeled exploratory. |
