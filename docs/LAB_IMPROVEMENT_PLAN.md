...
**Version:** 1.7
**Created:** 2026-07-10
**Updated:** 2026-09-08 -- R4/R5 full (change set 8) recorded
...
- [x] *Replay trigger (2026-09-08, change set 8):* `outcome_collect.REPLAY_TARGETS`
  declares regenerable derived artefacts; source `replay` regenerates each in
  a detached worktree at HEAD and opens a `FAIL` row on byte drift;
  `src/replay_check.py` is the same check for bisection and both gates; the
  declared outputs are the only tracked `results/` files a repair may
  regenerate (healer and gate exemption). Live: the stale
  `results/meta_uniformity.json` observed, bisected to 9488a9a, healed by
  the loop (see change set 8 plan §15).
- [ ] Source-drift fixtures (M4) and stale-hash detection for upstream inputs.
- [ ] Scheduled clean-checkout replay (M1) opens a row on any non-deterministic
  fixture hash (`replay` compares one regeneration with the committed bytes).
...
- [x] *Hash-linked lessons (2026-09-08, change set 8):* the lessons a repair
  brief injects are on the heal row and in the record's `agent.txt` as
  `lessons_sha256`, verbatim in `lessons.txt`.
- [x] *Re-tiering recommendation (2026-09-08, change set 8):*
  `agent_eval_dispatch --rolls N` and `src/lab_tier.py --recommend` (latest
  ≤ 5 rolls per eval and tier; a tier counts only when proven on every eval
  with ≥ 3 rolls; < 3 rolls = "insufficient rolls"). The recommendation is
  a `tier` lesson; the frontmatter edit stays a PR. Open: the proposal PR
  itself is not yet generated by the loop.
- [x] *Owner-reserved stops routed at once (2026-09-08):* `lab_gate.route_unhealable`.
...
| 8 | **R4 + R5, full**: replay trigger + declared regenerable scope; owner-reserved routing at once; hash-linked lessons; eval rolls + tier recommendation | R4, R5 | COMPLETE (minimal) -- 2026-09-08, `docs/plans/LAB_RSI_R45_IMPLEMENTATION_PLAN.md`; live: the stale meta panel observed, attributed, regenerated by the proposer (PR #34) and routed to the owner (issue #35) because the fix also touched a verifier. Open inside 8: upstream source-drift fixtures (M4), clean-checkout replay (M1), the tier-change PR generated by the loop, hypothesis-to-registration (moved to UTE set 10) |
...
| 1.7 | 2026-09-08 | R4/R5 full recorded COMPLETE (minimal), change set 8: `replay` source over `REPLAY_TARGETS` with `src/replay_check.py` as the one check for bisection and both gates; a declared output is the only tracked `results/` file a repair may regenerate; owner-reserved stops routed on the first attempt; lessons hash-linked on heal rows; `--rolls` and `src/lab_tier.py` (proven-tier rule, insufficient-rolls guard). Live: the stale meta panel healed by the proposer (PR #34) and routed to the owner (issue #35) because the regeneration also re-pinned a verifier's expected value; the self-approval bar held. Hypothesis-to-registration moved to UTE set 10. |
